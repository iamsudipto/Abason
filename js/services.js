...
const ASvc=(function(){
  const MOCK=true;
  class SErr extends Error{constructor(code,msg){super(msg);this.code=code;}}
  ...
  // OTP
  const otp=new Map();
  const isValidBDPhone=p=>{if(typeof p!=='string')return{ok:false};const d=p.replace(/[\s-]/g,'').replace(/^\+?880/,'0');return /^01[3-9]\d{8}$/.test(d)?{ok:true,normalised:d}:{ok:false};};
  ...
  // NID
  const isValidNIDFormat=n=>typeof n==='string'&&/^(\d{10}|\d{13}|\d{17})$/.test(n.trim());
